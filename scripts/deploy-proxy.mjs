...
async function dockerBuild() {
    $.cwd = path.join(__dirname, '..')
    const out = await $`docker build . -f Dockerfile.proxy`
    const a = out
        .toString()
        .split('\n')
...
async function dockerTagAndPush(a) {
    ...
    const tagged = `ccr.ccs.tencentyun.com/flowda-test/node-chatgpt-api-proxy:${head}`
    ...
    const dateString =
        ...
        m.getUTCSeconds()
    console.log(`
## node-chatgpt-api-proxy ${dateString}
是否 deploy: 否
镜像是否删除：否
分支：${branch}
commit: ${head}
